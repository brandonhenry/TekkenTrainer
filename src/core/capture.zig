...
pub const Capture = struct {
    ...
    pub fn captureFrame(self: *Self, game_memory: *const game.Memory) core.Frame {
    ...
    fn updatePreviousHitLines(self: *Self, game_memory: *const game.Memory) void {
        const maybe_hit_lines_1 = game_memory.player_1.findConstFieldPointer("hit_lines");
        self.previous_player_1_hit_lines = if (maybe_hit_lines_1) |l| l.* else null;
        const maybe_hit_lines_2 = game_memory.player_2.findConstFieldPointer("hit_lines");
        self.previous_player_2_hit_lines = if (maybe_hit_lines_2) |l| l.* else null;
    }

    fn captureLeftPlayerId(game_memory: *const game.Memory) core.PlayerId {
        if (game_memory.player_1.findConstFieldPointer("player_id")) |player_id| {
            return if (player_id.* == 0) .player_1 else .player_2;
        }
        if (game_memory.player_2.findConstFieldPointer("player_id")) |player_id| {
            return if (player_id.* == 0) .player_2 else .player_1;
        }
        return .player_1;
    }

    fn captureMainPlayerId(game_memory: *const game.Memory) core.PlayerId {
        if (game_memory.player_1.findConstFieldPointer("is_picked_by_main_player")) |is_main| {
            return if (is_main.*) .player_1 else .player_2;
        }
        if (game_memory.player_2.findConstFieldPointer("is_picked_by_main_player")) |is_main| {
    ...
    fn capturePlayer(
        self: *Self,
        player: *const memory.StructProxy(game.Player),
    ...
    fn capturePlayerPosition(player: *const memory.StructProxy(game.Player)) ?math.Vec3 {
        if (player.findConstFieldPointer("collision_spheres")) |spheres| {
            return spheres.lower_torso.convert().center;
        }
        if (player.findConstFieldPointer("hurt_cylinders")) |cylinders| {
    ...
    fn captureSkeletonLines(
        ...
        const cylinders: *const game.HurtCylinders = player.findConstFieldPointer("hurt_cylinders") orelse return 0;
        const spheres: *const game.CollisionSpheres = player.findConstFieldPointer("collision_spheres") orelse return 0;
    ...
    fn captureHitLines(
        ...
        const current_lines: *const game.HitLines = player.findConstFieldPointer("hit_lines") orelse return 0;
        var size: usize = 0;
