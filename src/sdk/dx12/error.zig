...
// This is based on the Microsoft's documentation on Direct3D 12 Return Codes and DXGI_ERROR.
...
pub const ErrorEnum = enum(w32.HRESULT) {
    S_OK = w32.S_OK,
    S_FALSE = w32.S_FALSE,
    D3D12_ERROR_ADAPTER_NOT_FOUND = w32.D3D12_ERROR_ADAPTER_NOT_FOUND,
    D3D12_ERROR_DRIVER_VERSION_MISMATCH = w32.D3D12_ERROR_DRIVER_VERSION_MISMATCH,
    E_FAIL = w32.E_FAIL,
...
test "from should return correct value when result code represents failure" {
    try testing.expectEqual(
        Error{ .result = w32.D3D12_ERROR_ADAPTER_NOT_FOUND },
        Error.from(w32.D3D12_ERROR_ADAPTER_NOT_FOUND),
    );
}
