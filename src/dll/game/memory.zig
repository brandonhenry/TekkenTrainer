...
pub const Memory = struct {
    ...
    pub fn init(
        ...
        const player_offsets = structOffsets(game.Player, .{
            ...
            .input = 0x2664,
            .hit_lines = 0x26D0,
            .hurt_cylinders = 0x2AD0,
            .collision_spheres = 0x2F10,
            .health = 0x3690,
        });
        const self = Self{
    ...
    fn initPatternCache(allocator: std.mem.Allocator, base_dir: ?*const sdk.io.BaseDir) !sdk.memory.PatternCache {
    ...
    fn deinitPatternCache(cache: *sdk.memory.PatternCache, base_dir: ?*const sdk.io.BaseDir) void {
        if (base_dir) |dir| {
    ...
    fn loadPatternCache(cache: *sdk.memory.PatternCache, base_dir: *const sdk.io.BaseDir) !void {
    ...
    fn savePatternCache(cache: *sdk.memory.PatternCache, base_dir: *const sdk.io.BaseDir) !void {
        var buffer: [sdk.os.max_file_path_length]u8 = undefined;
        const file_path = base_dir.getPath(&buffer, pattern_cache_file_name) catch |err| {
            sdk.misc.error_context.append("Failed to construct file path.", .{});
            return err;
